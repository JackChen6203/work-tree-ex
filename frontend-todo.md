...
- [x] 導入 `@dnd-kit/core` 拖拉排序套件
- [x] Itinerary items 支援拖拉重新排列
- [x] 支援跨日拖拉（從 Day 1 拖到 Day 2）
- [x] 拖拉過程中視覺 feedback（ghost card, drop indicator）
- [x] 拖拉完成後呼叫 reorder API + optimistic UI
- [x] 行動裝置觸控拖拉支援
- [x] 不支援 drag 時顯示上/下排序按鈕 fallback

---

## FE-P2-02｜真實地圖 SDK 整合

- [x] 整合 Mapbox GL JS 或 Google Maps JS SDK
- [x] `MapProviderAdapter` 實作連接真實 SDK
- [x] 地圖元件渲染 itinerary POI markers
- [x] 點擊 marker 聚焦對應 itinerary item
- [x] 點擊 itinerary item 平移地圖至該點
- [x] 路線 polyline 繪製
- [x] 大量點位 marker clustering（真實 SDK 層級）
- [x] SDK 載入失敗 → fallback 地址清單
...
- [x] 導入 `idb` 套件（IndexedDB wrapper）
- [x] 離線時 trip 資料寫入 IndexedDB 快取
- [x] Mutation queue 持久化到 IndexedDB（非純記憶體）
- [x] App 重啟後從 IndexedDB 恢復 pending mutations
- [x] IndexedDB 資料過期清理（>7 天）
- [x] API key 絕不寫入 IndexedDB

---

## FE-P2-05｜真實 FCM Push 註冊

- [x] Firebase SDK 初始化（`firebase/messaging`）
- [x] 請求推播權限 dialog
- [x] 取得 FCM token 並上傳至 backend `POST /fcm-tokens`
- [x] Token 過期時靜默刷新上傳
- [x] 前景推播處理（顯示 toast）
- [x] 背景推播處理（Service Worker notification）
- [x] 使用者拒絕推播 → 僅 in-app 通知
...
- [x] Must-visit / avoid 支援 tag input（新增/刪除 chips）
- [x] Wake pattern / POI density 滑桿選擇器
- [x] 提交後顯示真實 planning job 進度輪詢
...
- [x] Login magic link 流程 E2E
- [x] Trip 建立 → itinerary 編輯 → budget 設定 完整流程 E2E
...
> 來源：`quest.md` — 用戶反饋的 UX 改善需求

---

## FE-Q1｜Trip 建立 Wizard UX 強化

- [ ] 建立旅程表單新增「預算金額」欄位（數字輸入 + 幣別前綴)
- [ ] 建立旅程表單新增「出發地點」欄位
- [ ] 「目的地」改為 Autocomplete 下拉（呼叫 `/api/v1/places/autocomplete`）
- [ ] Autocomplete 最後一項顯示「沒有您想要的地方嗎？」→ 連結 Google Maps / AI 推薦
- [ ] 選取目的地後自動回填座標（`destinationLat` / `destinationLng`）
- [ ] 「時區」改為下拉選單（IANA timezone list）
- [ ] 選取目的地後自動推算時區（座標 → timezone API）
- [ ] 「幣別」改為下拉選單（ISO-4217，支援搜尋過濾）
- [ ] 常用幣別置頂（TWD, USD, JPY, EUR）

### 邊界個案
- [ ] 目的地搜尋 API 失敗 → fallback 純文字手動輸入
- [ ] 搜尋結果為空 → 顯示「請手動輸入」
- [ ] 自動推算時區失敗 → 保留手動選擇
- [ ] 幣別清單支援搜尋過濾
...
- [ ] 無 trip 時 itinerary / budget / map / ai 導覽灰化 + tooltip 說明
- [ ] Trip detail 載入失敗顯示友善錯誤 + 重試按鈕
- [ ] 404 → 「旅程不存在或已刪除」
- [ ] 403 → 「無權限存取此旅程」
- [ ] Timeout → 「載入逾時，請重試」
